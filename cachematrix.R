...
makeCacheMatrix <- function(x = matrix()) {
        ...
        set <- function(y) {
                ...
                z <<- NULL ## re-initialize c in the parent environment to null
        }
        get <- function() x ## return the matrix x
        setinverse <- function(inverse) z <<- inverse ## set the cache c equal to the inverse of the matrix x
...
## > z <- makeCacheMatrix(matrix(c(2, 0, 0, 2), c(2, 2)))
## > cacheSolve(z)
##      [,1] [,2]
## [1,]  0.5  0.0
## [2,]  0.0  0.5
